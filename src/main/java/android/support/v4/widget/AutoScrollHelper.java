...
public abstract class AutoScrollHelper implements OnTouchListener {
    ...
    private static class ClampedScroller {
        ...
        private float getValueAt(long j) {
            ...
            return (AutoScrollHelper.constrain(((float) (j - j2)) / ((float) this.mEffectiveRampDown), 0.0f, (float) AutoScrollHelper.DEFAULT_RELATIVE_VELOCITY) * this.mStopValue) + (AutoScrollHelper.DEFAULT_RELATIVE_VELOCITY - this.mStopValue);
        }
    ...
    private float computeTargetVelocity(int i, float f, float f2, float f3) {
        ...
        f4 *= f3;
        return i2 > 0 ? constrain(f4 * edgeValue, f5, f6) : -constrain(f4 * (-edgeValue), f5, f6);
    }
